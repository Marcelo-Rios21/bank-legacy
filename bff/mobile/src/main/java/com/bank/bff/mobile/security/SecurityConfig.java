...
public class SecurityConfig {
    ...
    UserDetailsService userDetailsService(
            @Value("${BFF_MOBILE_USERNAME}") String username,
            @Value("${BFF_MOBILE_PASSWORD}") String password,
            PasswordEncoder passwordEncoder) {
    ...
    SecretKey jwtSecretKey(
            @Value("${BFF_MOBILE_JWT_SECRET}") String encodedSecret) {

        byte[] secret = Base64.getDecoder().decode(encodedSecret);

        if (secret.length < 32) {
            throw new IllegalArgumentException(
                    "BFF_MOBILE_JWT_SECRET debe contener al menos 256 bits");
        }
